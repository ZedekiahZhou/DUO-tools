...
Version: 1.0.0
ToDo: 
    1. add para to anno_TSS
    2. binom.test; add para Cov, FDRcutoff, ...
    3. add mode QC!!!!!!
"""
...
group_global.add_argument("--mode", type=str, default="all", choices=["all", "preprocessing", "mapping", "call_m6Am"], 
                          help='Run which part of the pipeline: all, preprocessing, mapping or call_m6Am; separated by comma if multiple part supplied; default is all')
...
def fun_m6Am(bam, prx, args):
    ...
    cmd="python " + args.DUOdir + "/Pileup/anno_TSS.py -i " + ftss_anno # 加参数！！！！！！！； check filter与R的结果是否一致
    print(cmd, flush=True)
    subprocess.call(cmd, shell=True)
