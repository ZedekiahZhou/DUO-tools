...
if not args.skipStep1:
    ...
    for i in range(len(args.inputs)):
        ...
        df = df.filter(
            ...
            pl.col("Chr", "Sites", "Strand", "Gene", "AGcov", "Acov"),
        ...
        if i == 0:
            ...
            df_merged = df_merged.join(
                ...
                on=["Chr", "Sites", "Strand", "Gene"], how="outer", coalesce=True
    ...
    # output merged list
    df_merged.write_csv(args.output + ".used", separator='\t')
    df_merged_stat.write_csv(args.output + ".stat", separator='\t')
else:
...
df_passed.write_csv(args.output + ".passed", separator="\t")
df_merged_stat2.write_csv(args.output + ".stat2", separator='\t')
